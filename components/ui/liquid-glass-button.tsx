...
/**
 * `LiquidButton` com `asChild` quebra aqui: o Slot do Radix exige exatamente
 * um filho para clonar props, mas o `LiquidButton` sempre renderiza vários
 * irmãos (os divs decorativos + o filho) dentro do mesmo `Comp`. Isso não é
 * ajuste de projeto — é como o componente colado se comporta. Para link de
 * navegação (a maioria dos CTAs do site), esta variante monta a mesma
 * composição visual diretamente sobre `next/link`, sem passar por `Slot`.
 */
function LiquidLink({
  href,
  className,
  ...
  children,
  ...props
}: React.ComponentProps<typeof Link> & VariantProps<typeof liquidbuttonVariants>) {
  return (
    <Link
      href={href}
      data-slot="button"
...
export const metalButtonVariants = (
...
export const MetalButton = React.forwardRef<HTMLButtonElement, MetalButtonProps>(
  ({ children, className, variant = 'default', ...props }, ref) => {
    ...
    const [touch] = React.useState(isTouchDevice)

    const buttonText = children || 'Button'
    const variants = metalButtonVariants(variant, isPressed, isHovered, touch)
...
interface MetalLinkProps extends React.AnchorHTMLAttributes<HTMLAnchorElement> {
  href: string
  variant?: ColorVariant
}

/**
 * `MetalButton` não tem `asChild`, e boa parte dos CTAs do site são navegação
 * (`next/link`), não `<button>`. Esta variante replica a mesma composição
 * visual sobre um `Link`, para não perder a interatividade de hover/press em
 * cada lugar que precisa parecer botão mas navegar para outra rota.
 */
export const MetalLink = React.forwardRef<HTMLAnchorElement, MetalLinkProps>(
...
MetalLink.displayName = 'MetalLink'
